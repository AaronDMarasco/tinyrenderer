...
from .trtypes import vec2, vec3, vec4
...
class ModelV2:
    """This model diverged at Lesson 8 HW 2 to more accurately follow C++ vector sizes"""
    ...
    class Face:
        ...
        data: tuple[ModelV2.FaceEntry, ModelV2.FaceEntry, ModelV2.FaceEntry]
        group: str | None = field(default=None, kw_only=True)
        smooth: int | None = field(default=None, kw_only=True)

        def __getitem__(self: Self, idx: int) -> ModelV2.FaceEntry:
            return self.data[idx]

    comments: list[str] = field(init=False, default_factory=list)
    faces: list[ModelV2.Face] = field(init=False, default_factory=list)
    # Insert a dummy vertex point to make everything 1-based to match face's vertex index:
    texture_vs: list[vec3] = field(init=False, default_factory=lambda: [vec3(0, 0, 0)])  # CPP is vec2?
    vertices: list[vec4] = field(init=False, default_factory=lambda: [vec4(0, 0, 0, 0)])
    v_normals: list[vec4] = field(init=False, default_factory=lambda: [vec4(0, 0, 0, 0)])
    ...
    def add_vertex(self: Self, vertex: vec4) -> None:
        self.vertices.append(vertex)

    def add_v_normal(self: Self, vertex: vec4) -> None:
            ...
            for line_no, line in enumerate(ifile, 1):
                match line:
                    ...
                    case _ if m := VERTEX_RE.match(line):
                        res.add_vertex(vec4(float(m["x"]), float(m["y"]), float(m["z"]), 1))
                    ...
                    case _ if m := V_NORMAL_RE.match(line):
                        res.add_v_normal(vec4(float(m["x"]), float(m["y"]), float(m["z"]), 0).normalized)
                    case _:
